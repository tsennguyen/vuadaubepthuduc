...
const seedUsers = async (store: Firestore) => {
...
const seedPosts = async (store: Firestore) => {
  for (let i = 1; i <= 10; i++) {
...
const seedRecipes = async (store: Firestore) => {
  for (let i = 1; i <= 10; i++) {
...
const seedChats = async (store: Firestore) => {
  ...
  await dmRef.collection("messages").add({
    ...
    deletedAt: null,
    reactions: {},
    readBy: { [users[0].uid]: FieldValue.serverTimestamp() },
  ...
  await dmRef.collection("messages").add({
    ...
    deletedAt: null,
    reactions: {},
    readBy: { [users[1].uid]: FieldValue.serverTimestamp() },
    replyToMessageId: null,
    systemType: null,
  });
  await dmRef.update({
  ...
  await groupRef.collection("messages").add({
    ...
    deletedAt: null,
    reactions: {},
    readBy: { [users[2].uid]: FieldValue.serverTimestamp() },
    replyToMessageId: null,
    systemType: null,
  });
