...
const aiConfigCache: Record<string, CachedConfig> = {};
...
const DEFAULT_AI_CONFIGS: Record<string, AiConfig> = {
  search: {
    id: "search",
    name: "Goi y tim kiem",
  ...
  chat_moderation: {
    ...
    name: "AI duyệt chat",
    description:
      "Quét tin nhắn chat, phát hiện vi phạm và trả về bản tóm tắt đã mask.",
    model: defaultModel,
    systemPrompt: [
 ...
 */
export function clearAiConfigCache(featureId?: string) {
  if (featureId) {
...
export async function getAiConfigOrThrow(featureId: string): Promise<AiConfig> {
  const now = Date.now();
  ...
  if (cached && (now - cached.fetchedAt) < CACHE_TTL_MS) {
    ...
    if (!cached.config.enabled) {
      throw new HttpsError(
        "failed-precondition",
        `AI feature '${featureId}' is currently disabled by admin`
      );
    }
  ...
  if (!config) {
    throw new HttpsError(
      "failed-precondition",
      `AI config '${featureId}' is missing`
    );
  }
...
function normalizeConfig(
  ...
  const maxOutputTokens = toNumber(
  ...
  return {
    ...
    extraNotes:
      typeof data.extraNotes === "string" ? data.extraNotes : fallback?.extraNotes,
    model,
    systemPrompt:
    ...
    userPromptTemplate:
      typeof data.userPromptTemplate === "string" && data.userPromptTemplate.trim()
